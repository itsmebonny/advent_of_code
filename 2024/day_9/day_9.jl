...
function efficient_compress_memory(memory_dict::Dict{Int, Int})::Dict{Int, Int}
    ...
    while file_checked >= 0
        ...
        for free_space in sorted_free_spaces
            println("Checking if we can compress file ", file_checked, " with size ", file_size, " starting at key ", key, " in free space ", free_space, " with size ", free_spaces[free_space])
            if free_spaces[free_space] >= file_size && free_space < key
                for i in key:key + file_size - 1
                    memory_dict[i] = file_checked
                end
                free_spaces[free_space] -= file_size
            end
        end
        file_checked -= 1
    end
